    ...
    <div class="col-12 grid-margin" style="overflow: hidden;">
        <div class="card">
            <div class="card-body">
                ...
                <form class="form-sample" action="{{ route('surat.update', $surat->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                                ...
                                <div class="col-sm-12">
                                    <select class="form-select" name="penerima_id" id="penerima_id">
                                        @foreach ($pegawai as $data)
                                            <option value="{{ $data->nip }}" {{ $surat->penerima_id == $data->nip ? 'selected' : '' }}>
                                                {{ $data->nama }}
                                            </option>
                                        @endforeach
                                    </select>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="lampiran" name="lampiran" value="{{ $surat->lampiran }}" required>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="form-group">
                            ...
                            <textarea class="form-control" id="isi_surat" name="isi_surat" style="color: #000; height: 500px;">{{ $surat->isi_surat }}</textarea>
                        </div>
                    </div>

                    <div class="d-flex justify-content-between mt-4">
                        <button type="submit" class="btn btn-primary">Simpan Perubahan</button>
                        <a href="{{ route('surat.index') }}" class="btn btn-dark">Batal</a>
                    </div>
